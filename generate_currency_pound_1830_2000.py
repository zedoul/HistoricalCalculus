...
def UK_measure(amount, fromyear, toyear):
    ...
    if d1:
        ...
        ret += "- Labour price based on wage is " + str(price(d2)) + "£, 한화 " +str(price(1696.96*d2)) +"₩\n"
        ret += "- Income price based on GDP is " + str(price(d3)) + "£, 한화 " +str(price(1696.96*d3)) +"₩\n"
...
source = 'docs/currency/POUNDSTERLING_1830_2000.md'
f = open(source,'w')
txt = generate_markdown(1830,2000,2013)
print(txt, file=f)
f.close()
